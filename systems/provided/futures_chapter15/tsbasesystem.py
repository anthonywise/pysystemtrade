...
from sysportfolio.tsportfolio import tscsvPortfolio
...
from systems.tsaccount import TSAccount


def futures_system(data=None, config=None, tsportfolio=None, trading_rules=None, log_level="on"):
    """
    ...
    if data is None:
        data = tscsvFuturesData()

    if config is None:
        config = Config(
            "systems.provided.futures_chapter15.futuresconfig.yaml")

    if tsportfolio is None:
        tsportfolio = tscsvPortfolio()

    rules = Rules(trading_rules)

    system = System([TSAccount(), Account(), PortfoliosFixed(), PositionSizing(), FuturesRawData(), ForecastCombine(),
                     ForecastScaleCap(), rules], data, config, tsportfolio)
